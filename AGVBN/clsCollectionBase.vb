...
Friend Class clsCollectionBase
    ...
    Friend Sub New(ByVal Value As ActiveGanttVBNCtl, ByVal sObjectName As String)
        mp_oControl = Value
    ...
    Public ReadOnly Property m_oItem(ByVal v_lIndex As String, ByVal v_lErr1 As Integer, ByVal v_lErr2 As Integer, ByVal v_lErr3 As Integer, ByVal v_lErr4 As Integer) As Object
        Get
            ...
            If Not mp_oControl.StrLib.StrIsNumeric(v_lIndex) Then
                If v_lIndex = "" Then
                    mp_oControl.mp_ErrorReport(v_lErr1, "Invalid " & m_sObjectName & " object key, key cannot be an empty string", "ActiveGanttVBNCtl.cls" & m_sObjectName & "s.Get m_oItem")
                    Return Nothing
                End If
                lIndex = m_lFindIndexByKey(v_lIndex)
                If lIndex = -1 Then
                    mp_oControl.mp_ErrorReport(v_lErr2, m_sObjectName & " object not found, invalid key (""" & v_lIndex & """)", "ActiveGanttVBNCtl.cls" & m_sObjectName & "s.Get m_oItem")
                    Return Nothing
                End If
            Else
                lIndex = mp_oControl.StrLib.StrCLng(v_lIndex)
                If mp_oControl.StrLib.StrCStr(lIndex) <> v_lIndex Then
                    mp_oControl.mp_ErrorReport(v_lErr3, m_sObjectName & " object not found, invalid Index", "ActiveGanttVBNCtl.cls" & m_sObjectName & "s.Get m_oItem")
                    Return Nothing
                End If
                If lIndex < 1 Or lIndex > mp_aoCollection.Count() Then
                    mp_oControl.mp_ErrorReport(v_lErr4, m_sObjectName & " object not found, Index out of bounds", "ActiveGanttVBNCtl.cls" & m_sObjectName & "s.Get m_oItem")
                    Return Nothing
                End If
            End If
    ...
    Public Sub m_Add(ByVal r_oObject As Object, ByVal v_sKey As String, ByVal v_lErr1 As Integer, ByVal v_lErr2 As Integer, Optional ByVal v_bKeyRequired As Boolean = False, Optional ByVal v_lKeyError As Integer = 0)
        ...
        If mp_oControl.StrLib.StrIsNumeric(v_sKey) Then
            mp_oControl.mp_ErrorReport(v_lErr1, "Invalid " & m_sObjectName & " object key, key cannot be numeric", "ActiveGanttVBNCtl.cls" & m_sObjectName & "s.Add")
            Return
        End If
        If v_sKey <> "" Then
            If m_bIsKeyUnique(v_sKey) = False Then
                mp_oControl.mp_ErrorReport(v_lErr2, "Key is not unique in " & m_sObjectName & "s collection", "ActiveGanttVBNCtl.cls" & m_sObjectName & "s.Add")
                Return
            End If
        End If
        If v_sKey = "" And v_bKeyRequired = True Then
            mp_oControl.mp_ErrorReport(v_lKeyError, "A Key is required for all objects in the " & m_sObjectName & "s collection", "ActiveGanttVBNCtl.cls" & m_sObjectName & "s.Add")
            Return
        End If
    ...
    Public Sub m_Remove(ByVal v_sIndex As String, ByVal v_lErr1 As Integer, ByVal v_lErr2 As Integer, ByVal v_lErr3 As Integer, ByVal v_lErr4 As Integer)
        ...
        If Not mp_oControl.StrLib.StrIsNumeric(v_sIndex) Then
            If v_sIndex = "" Then
                mp_oControl.mp_ErrorReport(v_lErr1, "Invalid " & m_sObjectName & " object key, key cannot be an empty string", "ActiveGanttVBNCtl.cls" & m_sObjectName & "s.Remove")
                Return
            End If
            lIndex = m_lFindIndexByKey(v_sIndex)
            If lIndex = -1 Then
                mp_oControl.mp_ErrorReport(v_lErr2, m_sObjectName & " object not found, invalid key (""" & v_sIndex & """)", "ActiveGanttVBNCtl.cls" & m_sObjectName & "s.Remove")
                Return
            End If
        ...
        Else
            ...
            If mp_oControl.StrLib.StrCStr(lIndex) <> v_sIndex Then
                mp_oControl.mp_ErrorReport(v_lErr3, m_sObjectName & " object not found, invalid Index", "ActiveGanttVBNCtl.cls" & m_sObjectName & "s.Remove")
                Return
            End If
            If lIndex < 1 Or lIndex > mp_aoCollection.Count() Then
                mp_oControl.mp_ErrorReport(v_lErr4, m_sObjectName & " object not found, Index out of bounds", "ActiveGanttVBNCtl.cls" & m_sObjectName & "s.Remove")
                Return
            End If
            lRemovedIndex = lIndex
        End If
    ...
    Public Function m_oReturnArrayElementKey(ByVal v_sKey As String) As Object
        ...
        Else
            ...
            If lIndex <> -1 Then
                Return mp_aoCollection(lIndex - 1)
            Else
                mp_oControl.mp_ErrorReport(SYS_ERRORS.ERR_RETARRELEMKEY_G, "Key not found", "ActiveGanttVBNCtl.cls" & m_sObjectName & "s.m_oReturnArrayElementKey")
            End If
        End If
        Return Nothing
    End Function
    ...
    Public Sub mp_SetKey(ByRef sCurrentKey As String, ByVal sNewKey As String, ByVal ErrNumber As Integer)
        If m_bIgnoreKeyChecks = False Then
            If mp_oControl.StrLib.StrIsNumeric(sNewKey) Or (sNewKey <> sCurrentKey And m_bIsKeyUnique(sNewKey) = False) Then
                mp_oControl.mp_ErrorReport(ErrNumber, "Numeric or duplicate " & m_sObjectName & " object key", "ActiveGanttVBNCtl.cls" & m_sObjectName & ".Let Key")
                Return
            End If
        End If
        sCurrentKey = sNewKey
