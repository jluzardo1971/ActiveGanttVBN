...
Public Class clsMath
    ...
    Public Function DateTimeAdd(ByVal Interval As E_INTERVAL, ByVal Number As Integer, ByVal dtDate As AGVBN.DateTime) As AGVBN.DateTime
        Dim dtReturn As AGVBN.DateTime = New AGVBN.DateTime()
        ...
        dtReturn = New AGVBN.DateTime(dtBuff.Year, dtBuff.Month, dtBuff.Day, dtBuff.Hour, dtBuff.Minute, dtBuff.Second)
    ...
    Public Function DateTimeDiff(ByVal Interval As E_INTERVAL, ByVal dtDate1 As AGVBN.DateTime, ByVal dtDate2 As AGVBN.DateTime) As Integer
    ...
    Public Function GetXCoordinateFromDate(ByVal dtCoordinate As AGVBN.DateTime) As Integer
    ...
    Public Function GetDateFromXCoordinate(ByVal v_lXCoordinate As Integer) As AGVBN.DateTime
    ...
    Public Function DetectConflict(ByVal StartDate As AGVBN.DateTime, ByVal EndDate As AGVBN.DateTime, ByVal RowKey As String, ByVal ExcludeIndex As Integer, ByVal LayerIndex As String) As Boolean
        Dim oTask As clsTask
        ...
        If EndDate < StartDate Then
            Dim dtEndDate As AGVBN.DateTime = StartDate
            Dim dtStartDate As AGVBN.DateTime = EndDate
            StartDate = dtStartDate
            EndDate = dtEndDate
        End If
        lLayerIndex = mp_oControl.Layers.oCollection.m_lReturnIndex(LayerIndex, True)
        If (lLayerIndex = -1) Then
            mp_oControl.mp_ErrorReport(SYS_ERRORS.INVALID_LAYER_INDEX, "Invalid Layer Index", "ActiveGanttVBNCtl.mp_bDetectConflict")
            Return False
        End If
    ...
    Private Function NewDateTime(ByVal Year As Integer, ByVal Month As Integer, ByVal Day As Integer, ByVal Hour As Integer, ByVal Minute As Integer, ByVal Second As Integer) As AGVBN.DateTime
        Dim dtReturn As AGVBN.DateTime
        dtReturn = New AGVBN.DateTime(Year, Month, Day, Hour, Minute, Second)
        Return dtReturn
    End Function

    Public Function RoundDate(ByVal Interval As E_INTERVAL, ByVal Number As Integer, ByVal dtDate As AGVBN.DateTime) As AGVBN.DateTime
        ...
        ElseIf (Interval = E_INTERVAL.IL_MONTH) Then
            Select Case Number
                Case 1
                    Dim dtNextMonth As AGVBN.DateTime
                    dtDate = RoundDate(E_INTERVAL.IL_DAY, 1, dtDate)
                    lBuffer = dtDate.Day
                    dtDate.SecondFractionPart = 0
                    If lBuffer = 1 Then
    ...
    Friend Function mp_DateBlockVisible(ByVal dtIntStart As AGVBN.DateTime, ByVal dtIntEnd As AGVBN.DateTime, ByVal dtBaseDate As AGVBN.DateTime, ByVal yInterval As E_INTERVAL, ByVal lFactor As Integer) As Boolean
        Dim dtStart As AGVBN.DateTime
        Dim dtEnd As AGVBN.DateTime
        If lFactor > 0 Then
            dtStart = dtBaseDate
    ...
    Friend Sub mp_GenerateTimeBlocks(ByRef aTimeBlocks As ArrayList, ByVal dtIntStart As AGVBN.DateTime, ByVal dtIntEnd As AGVBN.DateTime)
        Dim i As Integer
        Dim oTimeBlock As clsTimeBlock
        For i = 1 To mp_oControl.TimeBlocks.Count
            ...
            If mp_bIsValidTimeBlock(oTimeBlock) = True Then
                ...
                ElseIf oTimeBlock.TimeBlockType = E_TIMEBLOCKTYPE.TBT_RECURRING Then
                    Dim dtCurrent As AGVBN.DateTime
                    Dim dtBase As AGVBN.DateTime
                    Dim dtTimeLineStart As AGVBN.DateTime
                    Dim dtTimeLineEnd As AGVBN.DateTime
                    Select Case oTimeBlock.RecurringType
                        Case E_RECURRINGTYPE.RCT_DAY
                            ...
                            dtCurrent = New AGVBN.DateTime(dtTimeLineStart.Year, dtTimeLineStart.Month, dtTimeLineStart.Day, 0, 0, 0)
                            Do While dtCurrent < dtTimeLineEnd
                                dtBase = New AGVBN.DateTime(dtCurrent.Year, dtCurrent.Month, dtCurrent.Day, oTimeBlock.BaseDate.Hour, oTimeBlock.BaseDate.Minute, oTimeBlock.BaseDate.Second)
    ...
    Private Sub mp_AddTimeBlock(ByRef aTimeBlocks As ArrayList, ByVal dtBase As AGVBN.DateTime, ByVal oTimeBlock As clsTimeBlock)
    ...
    Friend Function mp_CheckDuration(ByRef aTimeBlocks As ArrayList, ByVal dtStartDate As AGVBN.DateTime) As Integer
        Dim i As Integer
    ...
    Friend Function mp_GetStartDate(ByRef aTimeBlocks As ArrayList, ByRef bStartDateVerified As Boolean, ByRef dtStartDate As AGVBN.DateTime) As Boolean
    ...
    Friend Sub mp_GetEndDate(ByRef aTimeBlocks As ArrayList, ByVal lDurationInSeconds As Integer, ByRef dtStartDate As AGVBN.DateTime, ByRef dtEndDate As AGVBN.DateTime)
        Dim i As Integer
        Dim bInside As Boolean = False
        For i = 0 To aTimeBlocks.Count - 2
            Dim oTB1 As S_TIMEBLOCK
    ...
    Friend Sub mp_ValidateStartDate(ByRef aTimeBlocks As ArrayList, ByRef dtStartDate As AGVBN.DateTime)
        Dim i As Integer
        For i = 0 To aTimeBlocks.Count - 1
            Dim oTB1 As S_TIMEBLOCK
            oTB1 = aTimeBlocks(i)
    ...
    Friend Sub mp_ValidateEndDate(ByRef aTimeBlocks As ArrayList, ByRef dtEndDate As AGVBN.DateTime)
        Dim i As Integer
        For i = 0 To aTimeBlocks.Count - 1
            Dim oTB1 As S_TIMEBLOCK
            oTB1 = aTimeBlocks(i)
    ...
    Friend Sub mp_StandarizeInterval(ByRef dtIntStart As AGVBN.DateTime, ByRef dtIntEnd As AGVBN.DateTime)
        If dtIntStart < dtIntEnd Then
            Return
        End If
        Dim dtIntBuff As AGVBN.DateTime
        dtIntBuff = dtIntStart
        dtIntStart = dtIntEnd
        dtIntEnd = dtIntBuff
    End Sub

    Public Function GetEndDate(ByRef StartDate As AGVBN.DateTime, ByVal DurationInterval As E_INTERVAL, ByVal DurationFactor As Integer) As AGVBN.DateTime
        Dim EndDate As AGVBN.DateTime = New AGVBN.DateTime()
        Dim dtIntStart As AGVBN.DateTime
        Dim dtIntEnd As AGVBN.DateTime
        Dim bFinished As Boolean = False
    ...
    Friend Sub mp_GetTimeBlocks(ByRef aTimeBlocks As ArrayList, ByRef dtStartDate As AGVBN.DateTime, ByRef dtEndDate As AGVBN.DateTime)
        If mp_oControl.TimeBlocks.IntervalType = E_TBINTERVALTYPE.TBIT_AUTOMATIC Then
    ...
    Public Function CalculateDuration(ByRef dtStartDate As AGVBN.DateTime, ByRef dtEndDate As AGVBN.DateTime, ByVal DurationInterval As E_INTERVAL) As Integer
    ...
    Private Function mp_GetDuration(ByRef aTimeBlocks As ArrayList, ByVal dtStartDate As AGVBN.DateTime, ByVal dtEndDate As AGVBN.DateTime) As Integer
        Dim i As Integer
        Dim bInside As Boolean = False
        Dim lReturn As Integer = 0
        For i = 0 To aTimeBlocks.Count - 2
    ...
    Friend Function mp_QSPartitionTB(ByRef aTimeBlocks As ArrayList, ByVal StartIndex As Integer, ByVal EndIndex As Integer) As Integer
        ...
        Dim x As AGVBN.DateTime = oX.dtStart
